    ...
    def __init__(self,usr,pwd):
        ...
        try:
            ...
            if isinstance(data, str):
                ...
                self.usr_access_key = data['access_key']
        except Exception as e:
            print("%s 登陆失败，请重试"%(usr))
        print("%s 登陆成功"%(self.usr))
        usr_cookie = get_cookie(self.usr_access_key)
        self.cookies={}
        for line in usr_cookie.split(';'):   
...
for ac in f.readlines():
    ac = ac.split(" ")
    print("成功读取 %s 账号信息"%(ac[0]))
    acc = Account(ac[0],ac[1])
    acclist.append(acc)
while True:
    for acc in acclist:
        acc.lefttime()
